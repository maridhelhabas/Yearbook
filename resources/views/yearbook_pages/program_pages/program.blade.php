      ...
      <div class="col-lg-2 col-xl-8 " style="position: fixed; width:85%; height: 100%; margin-top: 50%;margin-left: 250px;margin-right: 10px;">
        <div class="card shadow mt-2"style=" width:100%;" >
          <div class="card-header bg-white d-flex justify-content-between align-items-center">
            <h3 class="text-primary">Registration / Program</h3>
            <button class="btn btn-warning text-white" data-bs-toggle="modal" data-bs-target="#addprogramModal" ><i
                class="fa fa-circle-plus"></i> Add Program</button>
          </div>
...
<div class="modal fade " id="addprogramModal" tabindex="-1" aria-labelledby="exampleModalLabel"
  ...
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content" style=" background-color: #DCDCDC;">
      ...
      <form action="{{ route('program.store') }}" method="POST" id="add_department_form" enctype="multipart/form-data">
        @csrf
        <div class="modal-body p-4" style=" background-color: #DCDCDC;">
          <div class="row">
            ...
            <div class="my-2">
              <label for="prog_id" class="text-dark">{{ __('Program ID') }}<span class="text-danger">*</span></label>
              <input type="number" id="program_id" name="program_id" class="form-control form-control2 @error('program_id') is-invalid @enderror" placeholder="" value="{{ old('program_id') }}" required readonly style="cursor:pointer">
              @error('program_id')
                <span class="invalid-feedback" role="alert">
                    <strong>{{ $message }}</strong>
                </span>
            ...
            <div class="my-2">
              <label for="program_name" class="text-dark">{{ __('Program Name') }}<span class="text-danger">*</span></label>
              <input type="text" name="program_name" class="form-control form-control2 @error('program_name') is-invalid @enderror" placeholder="Enter Program Name" value="{{ old('program_name') }}" required>
              @error('program_name')
                <span class="invalid-feedback" role="alert">
                    <strong>{{ $message }}</strong>
                </span>
...
<div class="modal fade " id="update_program" tabindex="-1" aria-labelledby="exampleModalLabel"
  data-bs-backdrop="static" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content dash-blade">
      ...
      <form action="{{ route('program.update', 'formAction') }}" method="POST"  enctype="multipart/form-data">
        @csrf
        @method('PUT') 
        <div class="modal-body dash-blade p-4 bg-light">
            <div class="col-lg">
              <label for="prog_id" class="text-dark">{{ __('Program ID') }}<span class="text-danger">*</span></label>
              <input type="number" name="program_id" class="form-control form-control2 @error('prog_id') is-invalid @enderror" placeholder="Enter Program ID" id="prog_id">
              @error('prog_id')
                <span class="invalid-feedback" role="alert">
                    <strong>{{ $message }}</strong>
                </span>
            ...
            <div class="my-2">
              <label for="prog_name" class="text-dark">{{ __('Program Name') }}<span class="text-danger">*</span></label>
              <input type="text" name="program_name" class="form-control form-control2 @error('prog_name') is-invalid @enderror" placeholder="Enter Program Name" id="prog_name" required>
              @error('prog_name')
                <span class="invalid-feedback" role="alert">
                    <strong>{{ $message }}</strong>
                </span>
          ...
          <div class="my-2">
              <label for="dept_name" class="text-dark">{{ __('Department') }}<span class="text-danger">*</span></label>
              <input type="text" name="department_name" class="form-control form-control2 @error('dept_name') is-invalid @enderror" placeholder="Enter Department" id="dept_name" required>
              @error('dept_name')
                <span class="invalid-feedback" role="alert">
                    <strong>{{ $message }}</strong>
                </span>
        ...
        <div class="modal-footer">
          <!-- <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button> -->
          <button type="submit"  class="btn btn-primary d-grip gap-2" style="width: 30%; border-color:white">Add Program</button>
        </div>
      </form>
    </div>
  </div>
...
let table = $('#example').DataTable({
    ajax: {
        url: '{{ route('program.fetch_program') }}',
        method: 'GET',
    ...
    columns: [
...

        {
            ...
            render: function(data, type, row) {
              var programId = row.program_id;
              var programName = row.program_name;
...
$(document).ready(function() {
    $('#example').on('click', '.editbtn_staff', function() {
        var programId = $(this).data('program-id'); // Use data-user-id to access user_id
    ...
    // document.getElementById('update_staff_form').setAttribute('action', formAction);
     ...
     var formAction = "{{ route('program.update', ':program_id') }}".replace(':program_id', programId);
...
<script>
  $(document).ready(function() {
      ...
      function fetchLastProgID() {
          $.ajax({
              url: '/fetch-last-prog-id',
              method: 'GET',
              success: function(data) {
  
                  if (data && data.lastProgId) {
                      $('#program_id').val(data.lastProgId);
                  } else {
      ...
      $('.btn[data-bs-target="#addprogramModal"]').on('click', function() {
