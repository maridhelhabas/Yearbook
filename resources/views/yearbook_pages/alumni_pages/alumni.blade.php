...
<body>
   ...
   <main class="main-container">
        ...
        <div class="col-lg-2 col-xl-8" style=" position: fixed; width:85%; height: 50%; margin-top: 28%;margin-left: 250px;margin-right: 10px;">
          <div class="card shadow mt-2" >
            <div class="card-header bg-white d-flex justify-content-between align-items-center">
              ...
              <button class="btn btn-warning text-white" data-bs-toggle="modal" data-bs-target="#addAlumniModal" ><i
    ...
    {{-- add new alumni modal start --}}
<div class="modal fade " id="addAlumniModal" tabindex="-1" aria-labelledby="exampleModalLabel"
  ...
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content" style=" background-color: #DCDCDC;">
      <div class="modal-header" style=" background-color: #DCDCDC;">
        <h5 class="modal-title" id="exampleModalLabel">New Alumni</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <form action="{{ route('alumni.store') }}" method="POST" id="add_staff_form" enctype="multipart/form-data">
        @csrf
        <div class="modal-body p-4" style=" background-color: #DCDCDC;">
          <div class="row">
            ...
            <div class="col-lg">
              <label for="user_id" class="text-dark">{{ __('User ID') }}</label>
              <input type="number" name="user_id" class="form-control form-control2" id="last_alumni_id" value="{{ old('user_id') }}" required readonly>
              
            </div>
            <div class="col-lg">
              <label for="first_name" class="text-dark">{{ __('First Name') }}<span class="text-danger">*</span></label>
              <input type="text" name="first_name" class="form-control form-control2 @error('first_name') is-invalid @enderror" placeholder="Enter First Name" value="{{ old('first_name') }}" required>
              @error('first_name')
                <span class="invalid-feedback" role="alert">
                    <strong>{{ $message }}</strong>
                </span>
            ...
            <div class="col-lg" style="display:none">
              ...
              <input type="text" name="role" class="form-control form-control2 @error('role') is-invalid @enderror" placeholder="Role" value="Alumni" required>
              @error('role')
                <span class="invalid-feedback" role="alert">
                    <strong>{{ $message }}</strong>
                </span>
            ...
            <div class="col-lg">
              ...
              <input type="text" name="last_name" class="form-control form-control2 @error('last_name') is-invalid @enderror" placeholder="Enter Last Name" value="{{ old('last_name') }}" required>
              @error('last_name')
                <span class="invalid-feedback" role="alert">
                    <strong>{{ $message }}</strong>
                </span>
          ...
          <div class="my-2">
            ...
            <input type="number"  name="phone_number" class="form-control form-control2 @error('phone_number') is-invalid @enderror" placeholder="Enter Phone Number" min="0" required>
            @error('phone_number')
                <span class="invalid-feedback" role="alert">
                    <strong>{{ $message }}</strong>
                </span>
...
{{-- update alumni moddal start --}}
<div class="modal fade " id="update_staff" tabindex="-1" aria-labelledby="exampleModalLabel"
  data-bs-backdrop="static" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered">
      ...
      <form action="{{ route('staff.update', 'formAction') }}" method="POST"  enctype="multipart/form-data">
        ...
        <div class="modal-body dash-blade p-4 bg-light">
          <div class="row">
            ...
            <div class="col-lg">
              <label for="user_id" class="text-dark">{{ __('User ID') }}<span class="text-danger">*</span></label>
              <input type="number" name="user_id" class="form-control form-control2 @error('user_id') is-invalid @enderror" placeholder="user id" id="staff_id"  required>
              @error('user_id')
                <span class="invalid-feedback" role="alert">
                    <strong>{{ $message }}</strong>
                </span>
            ...
            <div class="col-lg">
              <label for="first_name" class="text-dark">{{ __('First Name') }}<span class="text-danger">*</span></label>
              <input type="text" name="first_name" class="form-control form-control2 @error('first_name') is-invalid @enderror" placeholder="firstname" id="staff_first_name" required>
              @error('first_name')
                <span class="invalid-feedback" role="alert">
                    <strong>{{ $message }}</strong>
                </span>
            ...
            <div class="col-lg">
              <label for="last_name" class="text-dark">{{ __('Last Name') }}<span class="text-danger">*</span></label>
              <input type="text" name="last_name" class="form-control form-control2 @error('last_name') is-invalid @enderror" placeholder="lastname" id="staff_last_name" required>
              @error('last_name')
                <span class="invalid-feedback" role="alert">
                    <strong>{{ $message }}</strong>
                </span>
          ...
          <div class="my-2">
            <label for="phone_number" class="text-dark">{{ __('Phone Number') }} <span class="text-danger">*</span></label>
            <input type="number"  name="phone_number" class="form-control form-control2 @error('phone_number') is-invalid @enderror" placeholder="phone number" id="staff_phone_no" min="0" required>
            @error('phone_number')
                <span class="invalid-feedback" role="alert">
                    <strong>{{ $message }}</strong>
                </span>
          ...
          <div class="my-2">
            <label for="email" class="text-dark">{{ __('Email Address') }} <span class="text-danger">*</span></label>
            <input type="email" name="email" class="form-control form-control2" placeholder="email" id="staff_email" required>
            @error('email')
                <span class="invalid-feedback" role="alert">
                    <strong>{{ $message }}</strong>
                </span>
...
// function format(d) {
//     // Construct the URL to the user image by combining the base image URL and user-specific URL
//    var userImage = "{{ asset('assets/upload_image') }}/" + d.user_image; // Assuming d.user_id contains the user-specific part of the URL
//  URL

//     // Create the HTML to display the user image
//     var html = '<dl>' +
//         '<dt>User Image:</dt>' +
//         '<dd>' +
...
let table = $('#example').DataTable({
    ...
    //             var imageUrl = '{{ route("get.user.image", "") }}' + '/' + data;
...
$(document).ready(function() {
    ...
    function fetchLastStaffID() {
        $.ajax({
            url: '/fetch-last-staff-id',
            method: 'GET',
            success: function(data) {

                if (data && data.lastStaffId) {
                    $('#add_staff_id').val(data.lastStaffId);
                } else {
    ...
    $('.btn[data-bs-target="#addstaffModal"]').on('click', function() {
...
$(document).ready(function() {
    ...
    function fetchAlumniID() {
        $.ajax({
            url: '/fetch-last-alumni-id',
            method: 'GET',
            success: function(data) {

                if (data && data.last_alumni_id) {
                    $('#last_alumni_id').val(data.last_alumni_id);
                } else {
                    console.error('Invalid or missing response data.');
                }
            },
    ...
    $('.btn[data-bs-target="#addAlumniModal"]').on('click', function() {
...
<!-- <script>
    // Use AJAX to fetch the last user_id for 'Alumni'
    $.ajax({
        url: "{{ route('alumni.getLastID') }}", 
        type: "GET",
        success: function(data) {

            $('#user_id').val(data.last_alumni_id);
        },
        error: function() {
            console.error('Error fetching last alumni id');
        }
    });
</script> -->
